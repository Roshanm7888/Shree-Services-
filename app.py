...
# Custom Styling (Proper Rendering Fix)
st.markdown("""
    <style>
    ...
    /* Bill Styling - High Fidelity */
    .bill-container { background: white; border: 2px solid #333; padding: 40px; border-radius: 2px; font-family: 'Arial', sans-serif; max-width: 750px; margin: auto; color: #000; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
...
# 3. TOP NAVIGATION
st.markdown('<div class="main-header"><h1>Shree Services - Online GST & Tax Center</h1><p>Accounting, GST & Taxation Solutions</p></div>', unsafe_allow_html=True)
...
if st.session_state.page == "bill":
    ...
    if not df.empty:
        ...
        # FINAL RENDERED BILL (UNSAFE ALLOW HTML IS TRUE)
        bill_html = f"""
        ...
        st.markdown(f'<a href="https://wa.me/{c_row["Mobile Number"]}?text={urllib.parse.quote(wa_msg)}" target="_blank"><div style="background:#25d366; color:white; padding:15px; border-radius:10px; text-align:center; font-weight:bold; margin-top:20px; cursor:pointer;">📲 Send This Bill on WhatsApp</div></a>', unsafe_allow_html=True)

elif st.session_state.page == "home":
    st.info("Welcome to Shree Services Portal. Use top buttons to navigate.")
    for s in ["📊 Taxation", "🛡️ Insurance", "📝 Online Work"]:
        st.markdown(f'<div style="background:white; padding:20px; border-radius:10px; border-left:10px solid #1e3a8a; margin-bottom:15px; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);"><h3>{s}</h3><p>Professional services for your business growth.</p></div>', unsafe_allow_html=True)
    ...
    st.file_uploader("Upload Sales / Purchase Bills", accept_multiple_files=True)
    if st.button("Submit to Drive"):
        st.success("Documents successfully submitted to Roshan Ji's Google Drive!")
