...
# 1. Page Configuration - SIDEBAR KO DETAULT OPEN RAKHNE KE LIYE
st.set_page_config(
...
st.markdown("""
    ...
    .upload-card { background: #f0f4f8; padding: 20px; border-radius: 10px; border: 2px dashed #1e3a8a; font-weight: bold; color: #1e3a8a; }
    </style>
...
if choice == "🏠 Home Page":
    st.markdown("""
        <div class="main-header">
            <h1 style="margin-bottom:0;">Shree Services - Online GST & Tax Center</h1>
            <p>A Complete Hub for Accounting & Taxation Solutions</p>
        </div>
    """, unsafe_allow_html=True)
    
    # Large Service Boxes
    cols = [{"n": "📊 Taxation", "d": "GST, House Tax & Business Tax Filing."},
    ...
    for s in cols:
        ...
        with st.expander(f"Apply for {s['n']}"):
            ...
            r = st.text_area("Requirements", key=f"r_{s['n']}")
            if st.button(f"Submit {s['n']} Request"):
                wa_txt = f"Inquiry for {s['n']}\nMobile: {m}\nDetails: {r}"
                st.markdown(f'<a href="https://wa.me/917888273972?text={urllib.parse.quote(wa_txt)}" target="_blank"><div style="background:#25d366; color:white; padding:10px; text-align:center; border-radius:10px;">📲 Send to Roshan Ji</div></a>', unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        st.markdown(f"""
            <div style="border:2px solid #1e3a8a; padding:20px; border-radius:10px; background:white;">
                <h2 style="text-align:center; color:#1e3a8a;">Shree Services</h2>
                <p><b>To:</b> {party} <span style="float:right;">{datetime.now().strftime('%d-%b-%Y')}</span></p>
                <hr>
                <p>{particulars} <span style="float:right;"><b>₹{amt}/-</b></span></p>
                <div style="text-align:center; margin-top:20px;">
                    <img src="{qr}" width="130"><br><b>UPI: {FIXED_UPI}</b>
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    if mode == "sale" or mode == "all":
        st.markdown('<div class="upload-card">📁 UPLOAD SALE BILLS (GSTR-1)</div>', unsafe_allow_html=True)
        st.file_uploader("Select Sale Files", accept_multiple_files=True, key="sale")
        
    if mode == "purchase" or mode == "all":
        st.markdown('<div class="upload-card">📁 UPLOAD PURCHASE BILLS (GST-3B)</div>', unsafe_allow_html=True)
        st.file_uploader("Select Purchase Files", accept_multiple_files=True, key="pur")
        
    if st.button("Submit to Google Drive"):
        st.success("Files submitted! Please wait 1-2 minutes for them to appear in Drive.")
        st.balloons()
